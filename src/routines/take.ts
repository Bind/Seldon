...
import { createDelayedMove, createChainedMove } from "../index";
...
class PlanetConduitNode {
  ...
  getEnergyForMove(targetId) {
    ...
    return df.getEnergyNeededForMove(
...
export default function createTake(targetId, depthOfSearch = 3) {
  ...
  const target = df.getPlanetWithId(targetId);
  if (!target) {
    return;
  }
  const exclude = [];
  myPlanets.sort((a, b) => a.energyNeededToTake - b.energyNeededToTake); //Get least
